            ...
            {vouchers && vouchers?.length > 0 ? (
              vouchers?.map((voucher) => {
                ...
                return (
                  <div
                    key={voucher.id}
                    className="flex items-center bg-gradient-to-r from-pink-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden"
                  >
                    <div>
                      ...
                      <div className="relative z-10">
                        <h3 className="text-[1.4rem] font-bold mb-2 select-none">
                          {voucher.name}
                        </h3>

                        <div className="flex items-center gap-4 mb-4">
                          <div className="bg-white/20 text-[1.4rem] backdrop-blur-sm px-6 py-3 rounded-xl font-mono font-bold tracking-wider">
                            {voucher.voucherCode}
                          </div>
                          <button
                            onClick={() => handleCopycode(voucher)}
                            className="flex items-center text-[1.4rem] gap-2 bg-white text-pink-600 px-6 py-3 rounded-xl hover:bg-gray-100 transition shadow-md"
                          >
                        ...
                        <div className="text-[1.2rem] gap-2">
                    ...
                    {user && (
                      <div className="relative z-[100] ml-auto">
                        <button
                          className={`px-8 py-3 rounded-xl ${isSaved ? "border border-white text-white" : "bg-red-500 hover:bg-red-600"}  transition-colors duration-300 text-[1.4rem] cursor-pointer`}
                          onClick={() => {
                            if (user) {
