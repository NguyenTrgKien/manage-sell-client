...
import { faBoxOpen, faEdit, faHeart } from "@fortawesome/free-solid-svg-icons";
...
function CustomerProfile() {
  ...
  return (
    <div className={`px-[12rem] flex mt-[1rem]`}>
      <div className="w-[24rem] h-auto py-[1rem] px-[2rem]">
        <div className="flex items-center gap-[1rem] pb-[2rem] border-b border-b-gray-200">
          <div className="w-[5rem] h-[5rem] rounded-full border border-gray-400 flex items-center justify-center">
            {user?.avatar ? (
              <img
                src={user?.avatar}
                alt={`avatar`}
        ...
        <nav className="mt-[3rem] space-y-2">
          {navCustomerProfile.map((item) => {
            return (
              ...
              >
                <div className="flex items-center gap-[1rem] cursor-pointer group">
                  <FontAwesomeIcon
                    icon={item.icon}
                ...
                <div
                  className={`mt-[1.2rem] ${showChildren && showChildren.length > 0 && showChildren?.includes(item.id) ? "h-auto" : "h-[0rem] overflow-hidden"}  transition duration-300`}
                >
                  {item.children &&
                    item.children?.map((child) => {
                      ...
                      return (
                        <Link
                          to={child.path}
                          key={child.id}
                          className="block pl-[3rem] space-y-6 text-start text-gray-600 cursor-pointer group"
